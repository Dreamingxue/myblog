...
var {addArticle, myArticleCount} = require(path.join(__dirname, '../mysql/artical.js'))
...
router.post('/addArticle', function (req, res, next) {
  ...
  if (req.body.content == '') {
    ...
    //将发布的文章导入到数据库
    const {articleTitle, author, checkedType, keyword, content} = req.body;
    const params = [articleTitle, author, checkedType, keyword, content];
    addArticle((data) => {
      res.send('发布成功')
    }, (err) => {
      res.send('发布失败')
...
router.get('/alertArticle', function (req, res, next) {
  ...
  Article.update({_id: req.query._id}, {
    $set: {
      data: req.query.date,
      title: req.query.articleTitle,
      authod: req.query.authod,
      id: req.query.id,
      type: req.query.checkedType,
