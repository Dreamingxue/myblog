...
<script type="text/javascript">
    ...
    export default{
        ...
        //实例创建之后渲染数据到界面
        created(){
            axios.get('http://localhost:3000/users/personal').then((data)=>{
        ...
        methods:{
            //更改个人资料数据
            changePersonal(){
                axios.get('http://localhost:3000/users/changePersonal',{params:{nick:this.nick,address:this.address,email:this.email,_id:this._id}}).then((data)=>{
            ...
            //修改密码
            changePassword(){
                axios.post('http://localhost:3000/users/changePassword',{admin:this.nick,newPassword:this.newPassword,confirmPassword:this.confirmPassword}).then((data)=>{
                    this.pswmessage=data.data
                    this.pswToggle=true
                },(err)=>{
