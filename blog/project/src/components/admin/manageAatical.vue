...
<script type="text/javascript">
    ...
    export default{
        ...
        created(){
            ...
            //console.log(type)
            //获取所有文章数据
            axios.get('http://localhost:3000/myallArticle').then((data)=>{
        ...
        methods:{
            ...
            if(this.type=='所有分类'){
                axios.get(`http://localhost:3000/myallArticle/?page=${newIndex}`).then((data)=>{
                       this.articles=data.data;
                    },(err)=>{
                        this.message='查找失败'
                        this.show=true
                ...
                axios.get(`http://localhost:3000/selectType/?page=${newIndex}`,{params:{type:this.type}}).then((data)=>{
                        console.log('--------',data.data)
                       this.articles=data.data;
                    },(err)=>{
                        this.message='查找失败'
            ...
            selectType(){
                ...
                if(this.type=='所有分类'){
                    ...
                    axios.get('http://localhost:3000/selectType',{params:{type:this.type}}).then((data)=>{
                    //获取对象的长度
                   // console.log(Object.keys(data.data).length)
                    //this.total=Object.keys(data.data).length//获取对象的长度
                    this.articles=data.data
                    },(err)=>{
                        this.message='查找失败'
                        this.show=true
                    ...
                    //获取每种类型数量
                    axios.get('http://localhost:3000/selectTypeCount',{params:{type:this.type}}).then((data)=>{
                        //console.log(data.data)
                        this.total=data.data.length
                    },(err)=>{
                        this.message=='查找失败'
            ...
            //删除数据
            removeArticle(index){
                axios.get('http://localhost:3000/removeArticle',{params:{_id:this.articles[index]._id}}).then((data)=>{
            ...
            addTag(){
                 //添加标签
                axios.get('http://localhost:3000/addTags',{params:{tags:this.tags}}).then((data)=>{
                        if(data.data.code=1){
            ...
            removeTag(){
                 axios.get('http://localhost:3000/removeTags',{params:{tags:this.removeTags}}).then((data)=>{
                        if(data.data.code=1){
